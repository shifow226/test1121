...
public class UserDao {
	...
	// データ登録処理
	public void create(UserModel user, ActionMapping mapping, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
	...
	// 一覧取得処理
	public void list(UserModel user, ActionMapping mapping, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
	...
	// データ更新処理
	public void update(UserModel user, ActionMapping mapping, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
	...
	// データ削除処理
	public void delete(UserModel user, ActionMapping mapping, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
	...
	// データ詳細取得処理
	public UserModel show(UserModel user, ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {

		UserModel UserShowInfo = new UserModel();
		...
		try {
			...
			ps = con.prepareStatement(SHOWSQL);
			ps.setInt(1, user.getId());
			rs = ps.executeQuery();

			// データベースから取得した値をモデルに設定
			UserShowInfo = (UserModel) form;
			while (rs.next()) {
				UserShowInfo.setName(rs.getString("name"));
				UserShowInfo.setEmail(rs.getString("email"));
				UserShowInfo.setRemarks(rs.getString("remarks"));
			}
...
//			String remarks = request.getParameter("remarks");
			...
			// リクエストにUserShowInfoを設定（jsp側で使用）
			request.setAttribute("UserShowInfo", UserShowInfo);
